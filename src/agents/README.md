...
Business logic of the platform. Each agent is a Python class that consumes the
direct integrations (`PostgreSQLStore` today; later Qdrant, Neo4j, LLM /
embedding clients), invoked asynchronously via Celery tasks or synchronously
from the UI. Human-in-the-loop: users trigger or approve each step.
